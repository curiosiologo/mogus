...
import { RouterLink } from "vue-router";
  ...
  const fetchNinjas= async () => {
    ...
    const reportarmorte= async () => {
    const response = await fetch("http://localhost:8000/matar_ninja")
  };
...
<main class="bg--200 flex flex-row p-4 gap-8 box-shadow">
  <div class="w-full">
    ...
    <div class="flex justify-end">
        <router-link to="/mentor" class="space-mono-regular w-16 text-center align-middle bg-slate-600 -skew-x-12 backdrop-blur-md drop-shadow-3xl ring-1 ring-inset ring-red-400/40 text-white">Home</router-link>       
    </div>
    ...
    <div class="gap-4 flex flex-col mt-8">
        ...
        <button v-on:click="Convocar(selected_ninja, selected_report)" class="p-2 bg-gradient-to-br from-red-500/10 to-red-700/10 backdrop-blur-md border border-white/20 transform -skew-x-12 ring-1 ring-inset ring-red-400/40 drop-shadow-4xl text-xl bitcount-grid-double text-white">Convocar</button>
